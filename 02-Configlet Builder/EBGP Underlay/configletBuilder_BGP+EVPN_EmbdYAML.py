...
def configure_bgp():
  ...
  if "leaf" in hostname:
    ...
    mlag_peer = config[hostname]["MLAG"]["mlag_peer"]
    mlag_peer_ASN = config[mlag_peer]["BGP"]["ASN"]
    
    if config[hostname]["MLAG"]["type"] == "Even":
        mlag_peer_ip = "192.168.255.1"
    elif config[hostname]["MLAG"]["type"] == "Odd":
        mlag_peer_ip = "192.168.255.2"
...
# List the neighbor IPs here. It should be three spines for SPINE_Underlay and one LEAF_Peer
    ...
    print("  neighbor EVPN peer group")
    print("  neighbor EVPN remote-as %s") % spineASN
    ...
    if "DC1" in hostname:
        for peer in config["global"]["DC1"]["spine-peers"]:
            peer_ip = peer
            print("  neighbor %s peer group EVPN") % peer_ip
    elif "DC2" in hostname:
        for peer in config["global"]["DC2"]["spine-peers"]:
            peer_ip = peer
            print("  neighbor %s peer group EVPN") % peer_ip
